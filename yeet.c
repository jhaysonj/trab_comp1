...
int corrige_tentativa(int *tentativa, int total_digitos, int TAM);
...
int main(void) {
    Vetores vetor;
    int i,vidas=10,TAM,total_digitos = 42;
    char nickname[11];
    int correcao_tentativa = 42;
    ...
    while(vidas!=0) {
        ...
        correcao_tentativa = corrige_tentativa(vetor.tentativa, total_digitos, TAM);
        while(correcao_tentativa == 0) {
            printf("senha invalida.\n");
            printf("digite valores que estão no intervalo de 1 até %d.\n", total_digitos);
...
int corrige_tentativa (int *tentativa, int total_digitos, int TAM) {
    ...
    for(i = 0; i< TAM; i++) {
        if(tentativa[i] < 1 || tentativa[i] > total_digitos) {
            return 0;
        }
    }
    return 1;
